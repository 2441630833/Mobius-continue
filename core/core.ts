...
import { resolveChatModel } from "./config/selectedModels";
...
import { codebaseTool } from "./tools/definitions/codebaseTool";
import { fetchUrlContentTool } from "./tools/definitions/fetchUrlContent";
import { grepSearchTool } from "./tools/definitions/grepSearch";
import { readFileRangeTool } from "./tools/definitions/readFileRange";
import { searchWebTool } from "./tools/definitions/searchWeb";
import { resolveContinueToolName } from "./tools/toolNameAliases";
...
export class Core {
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("llm/complete", async (msg) => {
      const { config } = await this.configHandler.loadConfig();
      const model = config ? resolveChatModel(config) : null;
      if (!model) {
    ...
    on("llm/compileChat", async (msg) => {
      const { messages, options, title } = msg.data;
      const { config } = await this.configHandler.loadConfig();
      const model = config ? resolveChatModel(config, title) : null;
    ...
    on(
      ...
      async ({ data: { toolName, basePolicy, parsedArgs, processedArgs } }) => {
        ...
        const resolvedName = resolveContinueToolName(toolName) || toolName;
        const tool = config.tools.find((t) => t.function.name === resolvedName);
        if (!tool) {
        ...
        if (
          (resolvedName === "run_terminal_command" ||
    ...
    on("tools/preprocessArgs", async ({ data: { toolName, args } }) => {
      ...
      const resolvedName = resolveContinueToolName(toolName);
      if (!resolvedName) {
        return {
          preprocessedArgs: undefined,
          errorReason: ContinueErrorReason.Unknown,
          errorMessage:
            "Tool name was empty. Emit a native tool call with a registered tool name (e.g. read_file, ls, run_terminal_command).",
        };
      }
      ...
      if (!tool) {
        return {
          preprocessedArgs: undefined,
          errorReason: ContinueErrorReason.Unknown,
          errorMessage: `Tool "${toolName}"${resolvedName !== toolName ? ` (resolved to "${resolvedName}")` : ""} not found. Use a tool from the provided tools list.`,
        };
      }
      ...
      try {
        ...
        let errorMessage =
          e instanceof Error
            ? e.message
            : `Error preprocessing tool call args for ${resolvedName}\n${JSON.stringify(args)}`;
        return {
          preprocessedArgs: undefined,
          errorReason,
          errorMessage,
  ...
  private async handleToolCall(toolCall: ToolCall) {
    ...
    const resolvedName = resolveContinueToolName(requestedName);
    if (!resolvedName) {
      throw new Error(
        "Tool name was empty. Emit a native tool call with a registered tool name.",
      );
    }

    let tool = config.tools.find((t) => t.function.name === resolvedName);

    // Workbench Agent may advertise built-ins (e.g. codebase) even when they
    // were historically gated behind experimental flags — fall back to the
    // static definition so the call still executes.
    if (!tool) {
      const fallbacks = [
        codebaseTool,
        readFileRangeTool,
      ...
      tool = fallbacks.find((t) => t.function.name === resolvedName);
    }

    if (!tool) {
      throw new Error(
        `Tool ${requestedName}${resolvedName !== requestedName ? ` (→ ${resolvedName})` : ""} not found`,
      );
    }

    // Keep the resolved Continue name for callTool / policy lookups.
    ...
    // Bing / webpage GETs must not inherit LLM proxy or Authorization headers
    // from config.requestOptions — those break Search Web / Fetch Webpage.
    const isOutboundWebTool =
      resolvedName === BuiltInToolNames.SearchWeb ||
      resolvedName === BuiltInToolNames.FetchUrlContent;
    // Do not annotate init as DOM RequestInit — @continuedev/fetch uses node-fetch's RequestInit.
    const toolFetch = isOutboundWebTool
      ? (
          url: string | URL,
          init?: Parameters<typeof fetchwithRequestOptions>[1],
        ) => fetchwithRequestOptions(url, init, undefined)
      : (
          url: string | URL,
          init?: Parameters<typeof fetchwithRequestOptions>[1],
        ) => fetchwithRequestOptions(url, init, config.requestOptions);
