...
/** Skip connection attempts for untouched "Add MCP server" template configs. */
export function getUnconfiguredMcpServerMessage(
  options: InternalMcpOptions,
): string | undefined {
  if ("command" in options) {
    const serialized = JSON.stringify([options.command, ...(options.args ?? [])]);
    if (
      serialized.includes("<your-mcp-server>") ||
      serialized.includes("YOUR_MCP_SERVER") ||
      serialized.includes("<package-name>")
    ) {
      return `MCP server "${options.name}" is still using the default template. Edit the MCP config file, replace <your-mcp-server> with a real package (for example @modelcontextprotocol/server-filesystem), or delete this server.`;
    }
  }
  if ("url" in options) {
    const url = options.url ?? "";
    if (url.includes("<") || url.includes("example.com")) {
      return `MCP server "${options.name}" is still using the default template. Edit the server URL in the MCP config file, or delete this server.`;
    }
  }
  return undefined;
}
...
Org-level secrets can only be used for MCP by Background Agents (https://docs.continue.dev/hub/agents/overview) when \"Include in Env\" is enabled.`,
    ...
    const unconfiguredMessage = getUnconfiguredMcpServerMessage(this.options);
    if (unconfiguredMessage) {
      this.status = "error";
      this.errors.push(unconfiguredMessage);
      return;
    }
