...
import MCPConnection, { getUnconfiguredMcpServerMessage } from "./MCPConnection";
...
describe("MCPConnection", () => {
  ...
  describe("connectClient", () => {
    ...
    it("should skip connection for default MCP template placeholders", async () => {
      ...
      expect(conn.status).toBe("error");
      expect(conn.errors[0]).toContain("default template");
      expect(mockConnect).not.toHaveBeenCalled();
    });
  ...
  describe("getUnconfiguredMcpServerMessage", () => {
    it("detects stdio placeholder args", () => {
      const message = getUnconfiguredMcpServerMessage({
        name: "New MCP server",
        id: "New MCP server",
        type: "stdio",
        command: "npx",
      ...
      expect(message).toContain("default template");
    });

    it("returns undefined for configured stdio servers", () => {
      const message = getUnconfiguredMcpServerMessage({
        name: "Filesystem",
        id: "Filesystem",
        type: "stdio",
