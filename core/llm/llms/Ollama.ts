...
class Ollama extends BaseLLM implements ModelInstaller {
  ...
  static defaultOptions: Partial<LLMOptions> = {
    ...
    model: "codellama-7b",
    // Local llama-server is -np 1; large parallel batches wedge Ollama and hang chat.
  ...
  /**
   * Serialize embedding batches. BaseLLM.embed uses Promise.all across batches,
   * which floods the single local llama-server and can hang /api/chat.
   */
