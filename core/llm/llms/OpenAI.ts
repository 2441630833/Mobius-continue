...
class OpenAI extends BaseLLM {
  ...
  protected _getEndpoint(
    ...
    // `new URL(rel, base)` replaces the last path segment unless base ends with `/`.
    // Without this, `.../v1` + `chat/completions` becomes `.../chat/completions` (404).
    const apiBase = this.apiBase.endsWith("/")
      ? this.apiBase
      : `${this.apiBase}/`;
    ...
    if (this.apiType?.includes("azure")) {
      ...
      return new URL(`${path}${version}`, apiBase);
    }

    return new URL(endpoint, apiBase);
  }
  ...
  private _getEmbedEndpoint() {
    ...
    return new URL("embeddings", apiBase);
  }
