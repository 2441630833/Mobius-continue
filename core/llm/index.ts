...
export abstract class BaseLLM implements ILLM {
  ...
  private async parseError(resp: any): Promise<Error> {
    ...
    if (resp.status === 404 && !resp.url.includes("/v1")) {
      ...
      if (model && resp.url.match(/127\.0\.0\.1:25137|localhost:25137/)) {
    ...
    const customFetch = async (input: URL | RequestInfo, init: any) => {
      try {
        ...
        if (e.message.includes("/api/tags")) {
          ...
          if (
            e.code === "ECONNREFUSED" &&
            /127\.0\.0\.1:25137|localhost:25137/.test(e.message)
          ) {
  ...
  private canUseOpenAIResponses(options: CompletionOptions): boolean {
    const normalizedApiBase = this.apiBase?.replace(/\/+$/, "") + "/";
    const isOfficialOpenAIAPI =
      normalizedApiBase === "https://api.openai.com/v1/";
    return (
