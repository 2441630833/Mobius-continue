...
describe("prepareQueryForRipgrep", () => {
  it("should escape literal-looking queries", () => {
    ...
    expect(result.warning).toContain(
      "Escaped metacharacters for literal search",
    );
  });

  it("should escape incomplete quantifiers that ripgrep rejects", () => {
    // JS RegExp may accept `so{`, but ripgrep exits code 2 ("unclosed counted
    // repetition"). looksLikeLiteralSearch covers this case.
    const result = prepareQueryForRipgrep("so{");
    expect(result.query).toBe("so\\{");
    expect(result.warning).toMatch(
      /Escaped metacharacters for literal search|Invalid regex pattern/,
    );
  });

  it("should preserve intentional regex alternation", () => {
    const result = prepareQueryForRipgrep("sofil|solaser");
    expect(result.query).toBe("sofil|solaser");
  });
...
describe("patterns that should NOT be over-sanitized", () => {
  ...
  describe("patterns that should preserve user intent", () => {
    it("should not sanitize intentional regex alternation", () => {
      ...
      // `|` is preserved as intentional OR (common agent pattern)
