...
import { isLowQualitySessionTitle, sanitizeSessionTitle } from "./text";
...
export class ChatDescriber {
  ...
  static async describe(
    ...
    // Set the title
    const raw = removeQuotesAndEscapes(renderChatMessage(titleResponse));
  ...
  static async describeWithBaseLlmApi(
    ...
    try {
      ...
      if (titleResponse.choices && titleResponse.choices.length > 0) {
        ...
        if (content) {
          const cleaned = sanitizeSessionTitle(removeQuotesAndEscapes(content));
          if (!cleaned || isLowQualitySessionTitle(cleaned)) {
            return undefined;
          }
          return cleaned;
