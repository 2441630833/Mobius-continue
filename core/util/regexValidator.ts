 ...
 *
 * Agent queries are usually literal symbols / path fragments. Ripgrep always
 * treats `-e` as a regex, so unescaped metacharacters (especially incomplete
 * quantifiers like `so{`) cause exit code 2 and "Grep Search failed".
 *
 * Rules:
 * 1. Apply light sanitization (tabs, triple-backslashes, …).
 * 2. If the pattern does not compile as a JS RegExp, escape it as a literal.
 * 3. If it looks like a literal with metacharacters, escape it — except when
 *    it contains `|`, which agents commonly use as intentional alternation.
 */
export function prepareQueryForRipgrep(query: string): {
  ...
  let prepared = validation.sanitizedQuery || query;
  const warnings: string[] = validation.warning ? [validation.warning] : [];
  ...
  if (!compiles) {
    prepared = escapeLiteralForRegex(prepared);
    warnings.push(
      "Invalid regex pattern; escaped metacharacters for literal search",
    );
  } else if (looksLikeLiteralSearch(prepared) && !prepared.includes("|")) {
    // Preserve `foo|bar` as regex OR; escape other metacharacters (., (), [], …).
    prepared = escapeLiteralForRegex(prepared);
    warnings.push("Escaped metacharacters for literal search");
  }
