...
function getWindowsShell(): string {
  ...
  try {
    childProcess.execSync("pwsh -NoLogo -Command exit 0", { stdio: "ignore" });
...
/** PowerShell 5.x does not support `&&`; normalize for compatibility. */
...
function getShellCommand(command: string): { shell: string; args: string[] } {
  if (process.platform === "win32") {
    ...
    const normalized = normalizeWindowsCommand(command, shell);
    return {
      shell,
      args: ["-NoLogo", "-ExecutionPolicy", "Bypass", "-Command", normalized],
    };
  } else {
