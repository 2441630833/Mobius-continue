...
function resolveFetchUrl(args: Record<string, unknown>): string {
  if (args && typeof args.url === "string") {
    return getStringArg(args, "url");
  }
  if (args && Array.isArray(args.urls) && typeof args.urls[0] === "string") {
    return String(args.urls[0]).trim();
  }
  if (args && typeof args.uri === "string") {
    return getStringArg(args, "uri");
  }
  return getStringArg(args, "url");
}

export const fetchUrlContentImpl: ToolImpl = async (args, extras) => {
  const url = resolveFetchUrl(args as Record<string, unknown>);
  if (!url) {
    throw new Error(
      "`url` argument is required (or `urls` array with at least one URL).",
    );
  }
