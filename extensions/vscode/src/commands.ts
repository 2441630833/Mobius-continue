...
import {
  callBuiltInAgentTool,
  formatContextItemsAsText,
  getAgentChatTools,
  isClientEditTool,
  isCoreBuiltInTool,
} from "./agent/continueAgentTools";
import { applyClientEditTool } from "./agent/continueAgentPatchTools";
import { getAgentChatRules } from "./agent/continueAgentRules";
import {
  CONTINUE_CHAT_VIEW_ID,
  CONTINUE_SIDEBAR_VIEW_ID,
  focusContinueChatView,
} from "./util/chatViewId";
...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith(CONTINUE_SIDEBAR_VIEW_ID),
  );
}
...
const getCommandsMap: (
  ...
  return {
    ...
    "continue.dropFilesToChat": async (
      files: { path: string; title: string; isFolder: boolean }[],
    ) => {
      if (!files?.length) {
        console.log("[Continue DnD] dropFilesToChat: no files");
        return;
      }

      console.log(
        "[Continue DnD] dropFilesToChat received:",
        files.map((f) => f.path),
      );
      focusGUI();
      if (!sidebar.isReady) {
        const isReady = await waitForSidebarReady(sidebar, 5000, 100);
        if (!isReady) {
          console.log("[Continue DnD] sidebar not ready after timeout");
          return;
        }
      }
      ...
      const resolvedFiles = files.map((file) => {
        ...
        return {
          path: file.path,
          title: vscode.workspace.asRelativePath(uri, false) || file.title,
          isFolder: file.isFolder,
        };
      });

      console.log(
        "[Continue DnD] sending addToChat to webview:",
        resolvedFiles.length,
      );
      sidebar.webviewProtocol.send("addToChat", {
        data: resolvedFiles.map((file) => ({
    ...
    "continue.setFileDropOverlay": async (show: boolean) => {
      if (!sidebar.isReady) {
        return;
      }
      sidebar.webviewProtocol.send("setFileDropOverlay", { show: !!show });
    },
    ...
    /**
     * Agents model picker → modify provider.
     * Uses QuickInput (visible in native Agents). Also navigates Continue GUI
     * settings when that webview is available.
     */
    "continue.openModelProviderSettings": async () => {
      const { modifyModelProviderViaQuickPick } = await import(
        "./util/modelProviderQuickPick"
      );
      await modifyModelProviderViaQuickPick();
    },
    /**
     * Agents model picker → add provider.
     * QuickInput works in native Agents; webview navigate alone is a no-op there.
     */
    "continue.addModelProvider": async () => {
      const { addModelProviderViaQuickPick } = await import(
        "./util/modelProviderQuickPick"
      );
      await addModelProviderViaQuickPick();
    },
    /**
     * Agents model picker → delete provider.
     * QuickInput works in native Agents; webview navigate alone is a no-op there.
     */
    "continue.deleteModelProvider": async () => {
      const { deleteModelProviderViaQuickPick } = await import(
        "./util/modelProviderQuickPick"
      );
      await deleteModelProviderViaQuickPick();
    },
    ...
    /** Workbench Chat Agent: list OpenAI tool schemas (grep, web search, codebase, …). */
    "continue.getAgentChatTools": async () => getAgentChatTools(configHandler),
    ...
    "continue.callBuiltInTool": async (request: {
      ...
      toolCallId?: string;
      /** Session cwd URI string — scopes explore tools away from the open window folder. */
      ...
      if (!isCoreBuiltInTool(request.name)) {
        return {
          ok: false,
          contextItems: [],
          errorMessage: `Tool "${request.name}" is handled by the workbench, not Continue core`,
        };
      }
      const result = await withAgentWorkingDirectory(
        ide,
      ...
      return {
        ...
        text: formatContextItemsAsText(result.contextItems),
      };
    },

    /** Workbench Chat Agent: sync patch edit tools (find-replace, multi_edit, edit_existing_file). */
    "continue.applyClientEditTool": async (request: {
      name: string;
      arguments?: Record<string, unknown>;
      toolCallId?: string;
      workingDirectory?: string;
      ...
      if (!isClientEditTool(request.name)) {
        return {
          ok: false,
          text: `Tool "${request.name}" is not a client edit tool`,
          errorMessage: `Tool "${request.name}" is not a client edit tool`,
        };
      }
      const result = await withAgentWorkingDirectory(
        ide,
        request.workingDirectory,
        () => applyClientEditTool(request.name, request.arguments ?? {}, ide),
      );
      return {
        ok: result.ok,
        text: result.text,
...
/**
 * Temporarily override Continue IDE workspace dirs to the chat session cwd.
 * New Agent sessions can target a folder different from the open VS Code window;
 * without this, ls/grep/codebase/read_file silently search the wrong tree.
 */
async function withAgentWorkingDirectory<T>(
  ide: VsCodeIde,
  ...
  try {
    cwdUri = vscode.Uri.parse(workingDirectory);
  } catch {
    return run();
  }

  const utils = ide.ideUtils;
  const previous = utils.getWorkspaceDirectories();
  try {
    utils.setWokspaceDirectories([cwdUri]);
    return await run();
  } finally {
    utils.setWokspaceDirectories(previous);
  }
}
