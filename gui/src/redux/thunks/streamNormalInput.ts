...
export const streamNormalInput = createAsyncThunk<
  void,
  {
    ...
    depth?: number;
    /** Owning session — required for concurrent background agents. */
    sessionId?: string;
  },
  ThunkApiType
>(
  ...
  async (
    { legacySlashCommandData, depth = 0, sessionId: passedSessionId },
    ...
    // Repair malformed tool-call JSON in history so LiteLLM accepts follow-up turns.
    dispatch(sanitizeHistoryToolCallArguments());

    let selectedChatModel = selectChatModelForActiveSession(state);
    ...
    const agentAutoExecute = isAgentTaskAutoApproveEnabled(state3.session.mode);

    // 4. Execute remaining tool calls
    if (originalToolCalls.length === 0) {
      dispatch(setInactive());
    } else if (!agentAutoExecute && needsApprovalPolicies.length > 0) {
