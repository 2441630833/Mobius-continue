...
import { sanitizeSessionTitle } from "core/util/text";
import { ensureValidToolCallArguments } from "core/util/ensureValidToolCallArguments";
import {
  renderChatMessage,
  renderContextItems,
...
function findAssistantItemWithToolCall(
  ...
  for (let i = history.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      item.toolCallStates?.some((s) => s.toolCallId === toolCallId)
    ) {
      return item;
    }
  }
...
/** Keep toolCallStates and assistant message.toolCalls in sync with valid JSON. */
function syncSanitizedToolCallArguments(
  history: ChatHistoryItemWithMessageId[],
  toolCallState: ToolCallState,
): void {
  const sanitized = ensureValidToolCallArguments(
    toolCallState.toolCall.function?.arguments,
  );
  if (sanitized === toolCallState.toolCall.function?.arguments) {
    return;
  }

  toolCallState.toolCall.function.arguments = sanitized;

  const assistantItem = findAssistantItemWithToolCall(
    history,
    toolCallState.toolCallId,
  );
  if (assistantItem?.message.role === "assistant") {
    const msg = assistantItem.message as AssistantChatMessage;
    const tc = msg.toolCalls?.find((t) => t.id === toolCallState.toolCallId);
    if (tc?.function) {
...
function sanitizeAllToolCallArgumentsInHistory(
  history: ChatHistoryItemWithMessageId[],
): void {
  for (const item of history) {
    ...
    for (const toolCallState of item.toolCallStates) {
      syncSanitizedToolCallArguments(history, toolCallState);
    }
  }
}
...
export type SessionState = {
  ...
  mainEditorDraftContent?: JSONContent | undefined;
  symbols: FileSymbolMap;
  mode: MessageModes;
  isInEdit: boolean;
  /** Selected chat model title for this chat tab (per-chat AI provider). */
...
export const sessionSlice = createSlice({
  ...
  reducers: {
    ...
    setMainEditorDraftContent: (
      ...
      state.mainEditorDraftContent = action.payload;
    },
    clearMainEditorDraftContent: (state) => {
      state.mainEditorDraftContent = undefined;
    },
    ...
    newSession: (state, { payload }: PayloadAction<Session | undefined>) => {
      ...
      if (payload) {
        ...
        state.title = sanitizeSessionTitle(payload.title) || payload.title;
        state.id = payload.sessionId;
        state.chatModelTitle = payload.chatModelTitle ?? null;
        if (payload.mode) {
    ...
    /**
     * Swap the visible session without aborting any in-flight stream.
     * Used when parking a running agent to the background and showing another tab.
     */
    replaceSession: (
      state,
      { payload }: PayloadAction<SessionState | Session | undefined>,
    ) => {
      const previousId = state.id;

      if (!payload) {
        state.lastSessionId = previousId;
        state.history = [];
        state.title = NEW_SESSION_TITLE;
        state.id = uuidv4();
        state.isStreaming = false;
      ...
      // Parked SessionState (includes live AbortController)
      if ("streamAborter" in payload && "id" in payload) {
        const parked = payload as SessionState;
      ...
      state.lastSessionId = previousId;
      state.history = session.history as any;
      state.title = sanitizeSessionTitle(session.title) || session.title;
      ...
      state.contextPercentage = undefined;
      state.mainEditorDraftContent = undefined;
      state.chatModelTitle = session.chatModelTitle ?? null;
      if (session.mode) {
    ...
    setChatModelTitle: (
      ...
      state.chatModelTitle = payload ?? null;
    },
    updateSessionTitle: (state, { payload }: PayloadAction<string>) => {
      state.title = sanitizeSessionTitle(payload) || payload;
    },
    ...
    setToolGenerated: (
      ...
      if (toolCallState) {
        ...
        syncSanitizedToolCallArguments(state.history, toolCallState);
    ...
    sanitizeHistoryToolCallArguments: (state) => {
      sanitizeAllToolCallArgumentsInHistory(state.history);
    },
    cancelToolCall: (
      state,
      action: PayloadAction<{
...
export const {
  ...
  acceptToolCall,
  setToolGenerated,
  sanitizeHistoryToolCallArguments,
  updateToolCallOutput,
  setProcessedToolCallArgs,
  setMode,
  setIsSessionMetadataLoading,
