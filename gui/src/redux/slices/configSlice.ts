...
/**
 * Prefer the active (or stream-scoped) chat's bound model title so concurrent
 * tabs can use different AI API providers. Falls back to the global selection.
 */
export function selectChatModelForActiveSession(state: {
  session: { chatModelTitle?: string | null };
  config: ConfigState;
}) {
  const title = state.session.chatModelTitle;
  const chatModels = state.config.config.modelsByRole.chat ?? [];
  if (title) {
    const match = chatModels.find(
      (m) => m.title === title || m.model === title,
    );
    if (match) {
      return match;
    }
  }
  return state.config.config.selectedModelByRole.chat;
}
