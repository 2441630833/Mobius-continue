...
interface StatusIconProps {
  status: "running" | "completed" | "failed" | "background" | "pending";
}
...
interface UnifiedTerminalCommandProps {
  ...
  toolCallId?: string;
  displayLines?: number;
  /** When true, runs in chat panel only — no external VS Code terminal button */
  inlinePanel?: boolean;
  shellLabel?: string;
}
...
export function UnifiedTerminalCommand({
  ...
  if (isPending && !hasOutput) {
    ...
    statusType = "running";
  } else if (statusMessage?.includes("failed") || status === "failed") {
    statusType = "failed";
  ...
  return (
    <StyledTerminalContainer
      ...
      className="agent-terminal-panel mb-2 min-w-0 max-w-full"
      data-testid="terminal-container"
    >
      <div className="outline-command-border rounded-default bg-editor flex min-w-0 flex-col outline outline-1">
        {/* Toolbar */}
        <div
        ...
        >
          <div className="flex min-w-0 flex-row items-center gap-2">
            ...
            />
            <span className="text-description select-none">Terminal</span>
            <span className="agent-terminal-badge">{shellLabel}</span>
            {inlinePanel && (
              <span className="agent-terminal-badge agent-terminal-badge-inline">
                In chat panel
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2.5">
            {!isRunning && (
              <div className="xs:flex hidden items-center gap-2.5">
                ...
                {!inlinePanel && <RunInTerminalButton command={command} />}
              </div>
            )}
          </div>
        </div>
        ...
        {isExpanded && (
          <TerminalContent>
            <pre className="bg-editor agent-terminal-pre">
              <code>
                {/* Command with prompt */}
                <div className="text-terminal agent-terminal-command">
                  <span className="agent-terminal-prompt">PS&gt;</span>
                  <span>{command || "(waiting for command…)"}</span>
                </div>

                {/* Pending / running state */}
                {(isPending || (isRunning && !hasOutput)) && (
                  <div className="text-description mt-2 flex items-center gap-2 text-xs">
                    <BlinkingCursor />
                    <span>
                      {isPending
                        ? "Starting command in chat panel…"
                        : "Running…"}
                    </span>
                  </div>
                )}
        ...
        {/* Status information */}
        {(statusMessage || isRunning || isPending) && (
          <div
          ...
          >
            ...
            {isPending
              ? "Queued — will run in chat panel"
              : isRunning
                ? "Running in chat panel"
