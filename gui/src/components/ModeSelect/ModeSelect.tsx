...
export function ModeSelect() {
  ...
  const metaKeyLabel = useMemo(() => {
    ...
    // Defer so Headless UI Listbox (if open) doesn't steal focus back.
  ...
  const selectMode = useCallback(
    (newMode: MessageModes) => {
      ...
      // Headless UI Listbox restores focus to its button after onChange;
      // defer TipTap focus so it wins that race (otherwise caret disappears).
      setTimeout(() => {
        mainEditor?.commands.focus();
      }, 20);
    },
