...
import { restoreMainEditorDraft } from "../../../util/mainEditorDraft";
...
export const MainEditorProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const mainEditorDraftContent = useAppSelector(
    (store) => store.session.mainEditorDraftContent,
  );
  ...
  useEffect(() => {
    ...
    mainEditor,
    mainEditorContentTrigger,
    mainEditorDraftContent,
    dispatch,
  ]);
