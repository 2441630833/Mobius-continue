...
import { insertDroppedFiles } from "./utils/insertDroppedFiles";
...
function TipTapEditorInner(props: TipTapEditorProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      saveMainEditorDraft(dispatch, editor);
    };
  }, [editor, props.isMainInput, dispatch]);
  ...
  return (
    <InputBoxDiv
      ...
      onDrop={(event) => {
        ...
        const nativeFiles = droppedFiles
          .map((file) => {
        ...
        if (nativeFiles.length > 0 && editor) {
          void insertDroppedFiles(editor, ideMessenger, nativeFiles);
        }
      }}
    >
      <div className={`px-2.5 pb-1 ${props.isMainInput ? "pt-3" : "pt-2"}`}>
        <EditorContent
          className={`scroll-container overflow-y-scroll ${props.isMainInput ? "max-h-[70vh] min-h-[96px]" : ""}`}
          spellCheck={false}
          editor={editor}
          onClick={(event) => {
