...
To get started, [register](https://dataplatform.cloud.ibm.com/registration/stepone?context=wx) on watsonx SaaS, create your first project and setup an [API key](https://www.ibm.com/docs/en/mas-cd/continuous-delivery?topic=cli-creating-your-cloud-api-key).`,
  ...
  siliconflow: {
    ...
    description:
      "SiliconFlow OpenAI-compatible API for open-source models (also works with compatible gateways).",
    longDescription:
      "Obtain an API key from [SiliconFlow](https://cloud.siliconflow.cn/account/ak). Chat uses `POST {apiBase}/chat/completions` (apiBase must include `/v1`). Any OpenAI-compatible gateway works when you set API Base accordingly.",
    tags: [ModelProviderTags.RequiresApiKey, ModelProviderTags.OpenSource],
    params: {
