...
/** Mobius: auto-run all agent tools without Accept prompts (Cursor-like). */
export function applyPhysicalAiIdeAgentToolDefaults(dispatch: AppDispatch) {
  const keepDisabled = new Set<string>([
    BuiltInToolNames.CreateRuleBlock,
    BuiltInToolNames.RequestRule,
  ]);
  for (const toolName of Object.values(BuiltInToolNames)) {
    if (keepDisabled.has(toolName)) {
      continue;
    }
    dispatch(
      setToolPolicy({
